...
function display_to_events($date,$events){
	...
	foreach($events as $event){
		if($date == date('Y-m-d',strtotime($event["starttime"]))){
			$eventday =  "<br/>".Html::anchor("event/show/${event["id"]}",$event["title"]);
			return $eventday;
		}
	}
}
...
function getHolidays($year) {
	...
	$api_key = 'AIzaSyDFmu3IwAeWovN0w6xvubPSVXBvcoxfvRI';
	$holidays = array();
...
?>
	<td name=
	 ...
	 id=
	 	<?php 
		 if($i < count($display_tb_id_array)){
